...
function configureNxRelease(tree, semverProjects) {
    ...
    if (nxJson.release) {
        devkit_1.logger.info('Nx Release already configured, overwriting configuration.');
    }
    ...
    nxJson.release = {
        ...
        git: Object.assign({ commit: options.skipCommit != true }, (tree.exists('.husky') ? { commitArgs: '--no-verify' } : {})),
        changelog: {
