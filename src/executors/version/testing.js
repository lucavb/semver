...
function runInstall(dir) {
    (0, child_process_1.execSync)(`${packageManager} install --no-frozen-lockfile`, {
        cwd: dir,
        stdio: 'inherit',
    });
}
...
function setupTestingWorkspace() {
    ...
    return {
        runNx(command) {
            (0, child_process_1.execSync)(`pnpm exec nx ${command}`, {
                cwd: workspaceRoot,
                stdio: 'inherit',
                env: Object.assign(Object.assign({}, process.env), { NX_DAEMON: 'false' }),
            });
        },
        exec(command) {
        ...
        generateLib(name, options = '') {
            ...
            this.runNx(`g @nx/js:lib --directory=libs/${name} ${commonArgs} ${options}`);
        },
